...
function renderStats(){const stats=readStats();const top=items=>Object.entries(items||{}).sort((a,b)=>b[1]-a[1])[0]?.[0]||'—';const drink=$('#topDrink'),mood=$('#topMood'),rate=$('#completionRate'),share=$('#shareCount');if(drink)drink.textContent=top(stats.drinks);if(mood)mood.textContent=top(stats.moods);if(rate)rate.textContent=`${stats.started?Math.round(stats.completed/stats.started*100):0}%`;if(share)share.textContent=stats.shares}
function bumpChoice(type,value){const stats=readStats();const bucket=type==='drink'?stats.drinks:stats.moods;bucket[value]=(bucket[value]||0)+1;try{writeStats(stats)}catch{}renderStats()}
...
document.addEventListener('click',e=>{const c=e.target.closest('.choice');if(c){document.querySelectorAll(`[data-type="${c.dataset.type}"]`).forEach(x=>x.classList.remove('selected'));c.classList.add('selected');state[c.dataset.type]=c.dataset.value;bumpChoice(c.dataset.type,c.dataset.value);$('#revealBtn').disabled=!(state.drink&&state.mood)}});
document.querySelectorAll('[data-scroll-to]').forEach(b=>b.addEventListener('click',()=>{const target=document.getElementById(b.dataset.scrollTo);if(target)target.scrollIntoView({behavior:'smooth'})}));
...
async function showResult(spread=draw()){
  lastSpread=spread; $('#revealBtn').disabled=true; $('#revealBtn').textContent='牌面正在回应…';
async function showResult(spread=draw(), countStart=true){
  const isRetry=spread===lastSpread; lastSpread=spread; if(countStart&&!isRetry){const stats=readStats(); stats.started+=1; writeStats(stats); renderStats();} $('#revealBtn').disabled=true; $('#revealBtn').textContent='牌面正在回应…';
  ...
  $('#retryBtn').classList.add('hidden'); $('#resultActions').classList.add('hidden'); $('#coreSecret').classList.add('hidden'); $('#tarotSpread').innerHTML=''; $('#readingCopy').innerHTML='';
  $('#retryBtn').classList.add('hidden'); $('#resultActions').classList.add('hidden'); $('#feedback').classList.add('hidden'); $('#feedbackThanks').classList.add('hidden'); $('#coreSecret').classList.add('hidden'); $('#tarotSpread').innerHTML=''; $('#readingCopy').innerHTML='';
  $('#resultMeta').textContent=`${state.drink} · ${state.mood}`;
  $('#tarotSpread').innerHTML=spread.map(c=>`<article class="tarot-card ${c.reversed?'reverse':''}"><div class="tarot-inner"><div class="card-position">${c.role}</div><div class="card-symbol">${c.symbol}</div><div class="card-name">${c.name} ${c.reversed?'· 逆位':'· 正位'}</div></div></article>`).join('');
  try{
    const lines=await getAiReading(spread);
    $('#aiStatus').classList.add('hidden');
    $('#resultMeta').textContent=`${state.drink} · ${state.mood}`;
    $('#tarotSpread').innerHTML=spread.map(c=>`<article class="tarot-card ${c.reversed?'reverse':''}"><div class="tarot-inner"><div class="card-position">${c.role}</div><div class="card-symbol">${c.symbol}</div><div class="card-name">${c.name} ${c.reversed?'· 逆位':'· 正位'}</div></div></article>`).join('');
    const core=lines.join(' ').slice(0,150);
    ...
    $('#feedback').classList.remove('hidden'); const completedStats=readStats(); completedStats.completed+=1; writeStats(completedStats); renderStats();
  }catch(error){
    console.warn(error); $('#aiStatus').className='ai-status error'; $('#aiStatus').textContent='牌面暂时无法连接，请稍后重试。'; $('#resultActions').classList.remove('hidden'); $('#retryBtn').classList.remove('hidden');
    console.warn(error); const fallback=makeReading(spread); $('#aiStatus').className='ai-status error'; $('#aiStatus').textContent='AI 解读暂时无法连接，先为你展示牌面。'; $('#readingCopy').innerHTML=fallback.map((x,i)=>`<div class="reading-block"><h3>${spread[i].role} · ${spread[i].name} ${spread[i].reversed?'逆位':'正位'}</h3><p>${x}</p></div>`).join(''); window.currentText=`今日饮见 · ${state.drink} · ${state.mood}\n\n${fallback.join('\n\n')}`; $('#resultActions').classList.remove('hidden'); $('#retryBtn').classList.remove('hidden'); $('#feedback').classList.remove('hidden');
...
function createShareImage(){const stats=readStats();stats.shares+=1;writeStats(stats);renderStats();const canvas=document.createElement('canvas');canvas.width=1080;canvas.height=1350;const ctx=canvas.getContext('2d');const g=ctx.createLinearGradient(0,0,1080,1350);g.addColorStop(0,'#312044');g.addColorStop(.5,'#120d1d');g.addColorStop(1,'#08070d');ctx.fillStyle=g;ctx.fillRect(0,0,1080,1350);ctx.strokeStyle='rgba(215,179,106,.45)';ctx.lineWidth=2;ctx.strokeRect(46,46,988,1258);ctx.fillStyle='#d7b36a';ctx.font='28px serif';ctx.fillText('✦  今日饮见 · SIP & ARCANA',84,120);ctx.fillStyle='#f4ede4';ctx.font='48px serif';ctx.fillText(`${state.drink} · ${state.mood}`,84,210);ctx.font='34px serif';ctx.fillStyle='#d7b36a';ctx.fillText('今日三牌秘语',84,290);const cardNames=[...document.querySelectorAll('.card-name')].map(x=>x.textContent);cardNames.forEach((name,i)=>{ctx.fillStyle='#e4c995';ctx.font='30px serif';ctx.fillText(name,84,390+i*70)});ctx.fillStyle='#c6b9b8';ctx.font='26px serif';const words=(document.querySelector('#coreSecret p:last-child')?.textContent||'让今天的回应，成为温柔的方向。').slice(0,150);words.match(/.{1,24}/g)?.forEach((line,i)=>ctx.fillText(line,84,700+i*48));ctx.fillStyle='#827681';ctx.font='20px serif';ctx.fillText('仅供娱乐与自我探索，不代表确定的未来预测。',84,1240);const link=document.createElement('a');link.download='今日饮见-分享.png';link.href=canvas.toDataURL('image/png');link.click()}
...
try{renderStats()}catch(error){console.warn('Stats unavailable',error)}
